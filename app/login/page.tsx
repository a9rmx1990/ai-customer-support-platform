...
import { LogIn, User, ShieldCheck, Stethoscope, ArrowRight, CheckCircle2, Lock } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      setError('Account not found. Try one of the quick demo fill accounts below or register.');
    }
  };
      ...
      <div className="glass-panel p-8 rounded-2xl border border-gray-800 space-y-6 shadow-xl">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <button
              onClick={() => handleQuickLogin('alan@example.com')}
              className="w-full p-2.5 rounded-xl bg-gray-900/90 hover:bg-emerald-950/60 border border-gray-800 hover:border-emerald-500/40 text-left text-xs transition-all flex items-center justify-between text-gray-200"
            >
              <div>
                <p className="font-semibold text-teal-300">Alan Turing (Patient)</p>
                <p className="text-[10px] text-gray-500">alan@example.com • PAT-2002</p>
              </div>
